...
import {
  personalConfig, contactOrgConfig, experienceConfig, rulesConfig, securityConfig,
...
export default function StepPage() {
  ...
  const applicationTypeFromUrl = searchParams.get('type') ?? 'short_gc';
  ...
  const loadDraft = useCallback(async (appId: string) => {
    try {
      ...
      if (draft.governmentIdType === 'driver_license' && draft.governmentIdNumber && typeof draft.governmentIdNumber === 'string') {
  ...
  const saveDraft = useCallback(async () => {
    ...
    try {
      const endpoint = currentStep === 1 ? 'personal' : currentStep === 2 ? 'contact' : currentStep === 3 ? 'experience' : currentStep === 4 ? 'rules' : 'security';
      await fetch(`/api/applications/${applicationId}/${endpoint}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formData) });
  ...
  const handleBack = async () => {
    ...
    else router.push(`/test-application/${currentStep - 1}?id=${applicationId}&type=${applicationTypeFromUrl}`);
  };
  ...
  const handleStepComplete = async (payload: FormValues) => {
    ...
    if (updatedFormData.hasConfirmedDate !== 'yes') { delete updatedFormData.visitDate1; delete updatedFormData.visitDate2; delete updatedFormData.visitDate3; }
    ...
    try {
      if (currentStep === 1) {
        if (applicationIdFromUrl) {
          const response = await fetch(`/api/applications/${applicationId}/personal`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updatedFormData) });
          const result = await response.json();
          if (!response.ok) throw new Error(result.error);
          setLastSaved(new Date());
          router.push(`/test-application/2?id=${applicationId}&type=${applicationTypeFromUrl}`);
          return;
        } else {
          // ✅ Pass applicationTypeFromUrl to create route so DB gets the correct type
          const response = await fetch('/api/applications/create', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ applicationId, applicationType: applicationTypeFromUrl, ...updatedFormData }),
          });
          const result = await response.json();
          if (!response.ok) throw new Error(result.error);
          setLastSaved(new Date());
          window.history.replaceState(null, '', `/test-application/1?id=${applicationId}&type=${applicationTypeFromUrl}`);
          router.push(`/test-application/2?id=${applicationId}&type=${applicationTypeFromUrl}`);
          return;
        }
      }
      ...
      if (currentStep === 2) {
        sessionStorage.setItem(`app_${applicationId}_visitDates`, JSON.stringify({
          ...
          visitDate1: updatedFormData.visitDate1, visitDate2: updatedFormData.visitDate2, visitDate3: updatedFormData.visitDate3,
        }));
        const response = await fetch(`/api/applications/${applicationId}/contact`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updatedFormData) });
        if (!response.ok) { const result = await response.json(); throw new Error(result.error); }
        setLastSaved(new Date());
        router.push(`/test-application/3?id=${applicationId}&type=${applicationTypeFromUrl}`);
        return;
      }

      if (currentStep === 3) {
        const response = await fetch(`/api/applications/${applicationId}/experience`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updatedFormData) });
        if (!response.ok) { const result = await response.json(); throw new Error(result.error); }
        setLastSaved(new Date());
        router.push(`/test-application/4?id=${applicationId}&type=${applicationTypeFromUrl}`);
        return;
      }

      if (currentStep === 4) {
        const response = await fetch(`/api/applications/${applicationId}/rules`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updatedFormData) });
        if (!response.ok) { const result = await response.json(); throw new Error(result.error); }
        setLastSaved(new Date());
        router.push(`/test-application/5?id=${applicationId}&type=${applicationTypeFromUrl}`);
        return;
      }
      ...
      if (currentStep === 5) {
        ...
        const securityResponse = await fetch(`/api/applications/${applicationId}/security`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(securityDataWithoutFiles) });
        if (!securityResponse.ok) { const result = await securityResponse.json(); setIsSubmitting(false); throw new Error(result.error); }
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#f9f8f6' }}>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-white rounded-2xl p-8 shadow-2xl max-w-md mx-4">
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-sm text-center" style={{ color: '#1C3D5A' }}>Please wait while we process your information. This may take a moment.</p>
            </div>
          </div>
        </div>
      )}
              ...
              {STEP_CONFIGS.map((step) => (
                <div key={step.num} className="flex flex-col items-center" style={{ width: `${100 / STEP_CONFIGS.length}%` }}>
                  <div className="flex items-center justify-center w-10 h-10 rounded-full font-semibold mb-2 relative z-10"
                    style={step.num < currentStep ? { backgroundColor: '#355F7A', color: '#ffffff' } : step.num === currentStep ? { backgroundColor: '#1C3D5A', color: '#ffffff' } : { backgroundColor: '#E6E1D8', color: '#1C3D5A' }}>
                    {step.num < currentStep ? '✓' : step.num}
                  </div>
                  <span className="text-xs text-center" style={step.num <= currentStep ? { color: '#1F2933', fontWeight: '500' } : { color: '#9ca3af' }}>{step.label}</span>
                </div>
              ))}
            </div>
      ...
      <SectionForm config={currentConfig.config} initialValues={formData} onSubmit={handleStepComplete} onBack={handleBack} currentStep={currentStep} />
    </div>
  );
}